...
def debug_user_list(users_list: List[User]):
    [logger.info(json.dumps(asdict(o), indent=4, sort_keys=True, default=str)) for o in users_list]
...
if __name__ == "__main__":
    ...
    logger.info(f"user length {len(users)}")

    inserted_user_count = insert_user_list(users)
    logger.info("Inserted %s connections" % inserted_user_count)
