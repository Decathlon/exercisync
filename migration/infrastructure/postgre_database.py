...
DEFAULT_REDIRECT_LOCATION = "account.prodv1.decathlon.com"
...
def build_values(connections: List[Connection], partner_id_dict) -> list[tuple]:
    ag_engine = AES_GCM_Engine(AES_GCM_KEY)

    connection_query_values = []
    ...
    for connection in connections:
        ...
        connection_query_values.append(query_values)

    return connection_query_values
...
def insert_connection(connections: List[Connection], partner_ids: dict):
    ...
    with psycopg.connect(POSTGRES_HOST_API) as conn:
        ...
        with conn.cursor() as cur:
            try:
                cur.executemany(INSERT_QUERY, build_values(connections, partner_ids))
            except Exception as e:
                logging.error(f"Failed to push connections from HUB v1 user id, ERROR : {e}")
